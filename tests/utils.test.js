...
describe("extractEmailSignals", () => {
  test("detects attachment via content-disposition header", () => {
    const full = {
      headers: {},
      parts: [{ contentType: "text/plain", body: "hello", headers: { "content-disposition": ["attachment; filename=\"doc.pdf\""] } }],
    };
    const signals = extractEmailSignals(full, null);
    expect(signals.hasAttachments).toBe(true);
  });

  test("detects non-text/non-multipart part as attachment", () => {
    const full = {
      headers: {},
      parts: [
  ...
  test("detects text/calendar as calendar attachment", () => {
    const full = {
      headers: {},
      parts: [
        { contentType: "text/plain", body: "meeting" },
        { contentType: "text/calendar", body: "BEGIN:VCALENDAR" },
      ],
    };
    const signals = extractEmailSignals(full, null);
    expect(signals.hasCalendarAttachment).toBe(true);
  });

  test("calendar attachment implies hasAttachments", () => {
    const full = {
      headers: {},
      parts: [{ contentType: "text/calendar", body: "BEGIN:VCALENDAR" }],
    };
    const signals = extractEmailSignals(full, null);
    expect(signals.hasAttachments).toBe(true);
    expect(signals.hasCalendarAttachment).toBe(true);
  ...
  test("recipientCount from recipients + ccList", () => {
    const message = {
      recipients: ["a@example.com", "b@example.com"],
      ccList: ["c@example.com"],
    };
    const signals = extractEmailSignals(null, message);
    expect(signals.recipientCount).toBe(3);
  });

  test("isReply from in-reply-to header", () => {
    const full = { headers: { "in-reply-to": ["<abc@example.com>"] } };
    const signals = extractEmailSignals(full, null);
    expect(signals.isReply).toBe(true);
  });

  test("isReply from references header", () => {
    const full = { headers: { "references": ["<abc@example.com> <def@example.com>"] } };
    const signals = extractEmailSignals(full, null);
    expect(signals.isReply).toBe(true);
  });

  test("isMailingList from list-unsubscribe", () => {
    const full = { headers: { "list-unsubscribe": ["<mailto:unsub@example.com>"] } };
    const signals = extractEmailSignals(full, null);
    expect(signals.isMailingList).toBe(true);
  });

  test("isMailingList from list-id", () => {
    const full = { headers: { "list-id": ["<mylist.example.com>"] } };
    const signals = extractEmailSignals(full, null);
    expect(signals.isMailingList).toBe(true);
  });

  test("isAutoGenerated from auto-submitted header", () => {
    const full = { headers: { "auto-submitted": ["auto-generated"] } };
    const signals = extractEmailSignals(full, null);
    expect(signals.isAutoGenerated).toBe(true);
  });

  test("auto-submitted: no → isAutoGenerated false", () => {
    const full = { headers: { "auto-submitted": ["no"] } };
    const signals = extractEmailSignals(full, null);
    expect(signals.isAutoGenerated).toBe(false);
  });

  test("null/missing full and message → all-false defaults", () => {
    const signals = extractEmailSignals(null, null);
  ...
  test("plain text-only email has no attachments", () => {
    const full = {
      headers: {},
    ...
    expect(signals.hasCalendarAttachment).toBe(false);
  });
});
...
describe("formatSignalsLine", () => {
  test("formats all-false defaults", () => {
    const line = formatSignalsLine({
      hasAttachments: false, hasCalendarAttachment: false,
      recipientCount: 0, isReply: false, isMailingList: false, isAutoGenerated: false,
    });
    expect(line).toBe("Metadata: attachments=no, calendar-invite=no, recipients=0, reply-thread=no, mailing-list=no, auto-generated=no");
  });

  test("formats mixed signals", () => {
    const line = formatSignalsLine({
      hasAttachments: true, hasCalendarAttachment: true,
      recipientCount: 5, isReply: true, isMailingList: false, isAutoGenerated: false,
    });
    expect(line).toContain("attachments=yes");
    expect(line).toContain("calendar-invite=yes");
    expect(line).toContain("recipients=5");
    expect(line).toContain("reply-thread=yes");
    expect(line).toContain("mailing-list=no");
  });

  test("returns empty string for null/undefined", () => {
    expect(formatSignalsLine(null)).toBe("");
    expect(formatSignalsLine(undefined)).toBe("");
  });
});

// ---------------------------------------------------------------------------
// buildCatalogPrompt with signals
// ---------------------------------------------------------------------------
describe("buildCatalogPrompt with signals", () => {
  const body = "Budget report attached.";
  const subject = "Q1 Budget";
  const author = "alice@example.com";

  test("includes metadata line when signals provided", () => {
    const signals = {
      hasAttachments: true, hasCalendarAttachment: false,
      recipientCount: 3, isReply: false, isMailingList: false, isAutoGenerated: false,
    };
    const prompt = buildCatalogPrompt(body, subject, author, [], signals);
    expect(prompt).toContain("Metadata: attachments=yes");
    expect(prompt).toContain("recipients=3");
  });

  test("omits metadata line when signals is null", () => {
    const prompt = buildCatalogPrompt(body, subject, author, [], null);
    expect(prompt).not.toContain("Metadata:");
  });

  test("backward compat — works without signals argument", () => {
    const prompt = buildCatalogPrompt(body, subject, author, []);
    expect(prompt).not.toContain("Metadata:");
    expect(prompt).toContain('"tags"');
  });
...
// ---------------------------------------------------------------------------
// buildCombinedExtractionPrompt with signals
// ---------------------------------------------------------------------------
describe("buildCombinedExtractionPrompt with signals", () => {
  const body = "Let's meet Thursday at 3pm.";
  const subject = "Meeting Thursday";
  const author = "Alice <alice@example.com>";
  const mailDt = "02/20/2026";
  const currentDt = "02/20/2026";
  ...
  test("includes metadata line when signals provided", () => {
    const signals = {
      hasAttachments: false, hasCalendarAttachment: true,
      recipientCount: 1, isReply: true, isMailingList: false, isAutoGenerated: false,
    };
    const prompt = buildCombinedExtractionPrompt(body, subject, author, mailDt, currentDt, [], null, [], signals);
    expect(prompt).toContain("Metadata: attachments=no");
    expect(prompt).toContain("calendar-invite=yes");
    expect(prompt).toContain("reply-thread=yes");
  });

  test("omits metadata line when signals is null", () => {
    const prompt = buildCombinedExtractionPrompt(body, subject, author, mailDt, currentDt, [], null, [], null);
    expect(prompt).not.toContain("Metadata:");
  });
