...
// Extract structured metadata signals from the MIME tree and message headers.
// Pure function — no browser APIs. Returns an object with boolean/integer fields.
function extractEmailSignals(full, message) {
  const defaults = {
  ...
  if (!full && !message) return defaults;

  const signals = { ...defaults };

  // Walk MIME tree for attachment and calendar signals
  if (full) {
    function walkParts(part) {
      if (!part) return;
      // Check content-disposition header for attachments
      const disp = part.headers && part.headers["content-disposition"];
      if (disp) {
        const dispStr = Array.isArray(disp) ? disp[0] : disp;
        if (/attachment/i.test(dispStr)) {
          signals.hasAttachments = true;
        }
      }
      // Check for non-text/non-multipart parts (inline images, etc.)
      if (part.contentType &&
      ...
      if (part.contentType === "text/calendar") {
        signals.hasCalendarAttachment = true;
        signals.hasAttachments = true;
      }
    ...
    if (inReplyTo || references) {
      signals.isReply = true;
    }

    const listUnsub = headers["list-unsubscribe"];
    const listId = headers["list-id"];
    if (listUnsub || listId) {
      signals.isMailingList = true;
    }

    const autoSubmitted = headers["auto-submitted"];
    if (autoSubmitted) {
      const val = Array.isArray(autoSubmitted) ? autoSubmitted[0] : autoSubmitted;
      if (val && val.toLowerCase() !== "no") {
        signals.isAutoGenerated = true;
      }
    }
  }

  // Recipient count from message object
  if (message) {
    const toCount = Array.isArray(message.recipients) ? message.recipients.length : 0;
    const ccCount = Array.isArray(message.ccList) ? message.ccList.length : 0;
    signals.recipientCount = toCount + ccCount;
  }

  return signals;
}

// Format signals as a human-readable metadata line for injection into prompts.
function formatSignalsLine(signals) {
  if (!signals) return "";
  return `Metadata: attachments=${signals.hasAttachments ? "yes" : "no"}, calendar-invite=${signals.hasCalendarAttachment ? "yes" : "no"}, recipients=${signals.recipientCount}, reply-thread=${signals.isReply ? "yes" : "no"}, mailing-list=${signals.isMailingList ? "yes" : "no"}, auto-generated=${signals.isAutoGenerated ? "yes" : "no"}`;
}
...
function buildCatalogPrompt(emailBody, subject, author, existingTags, signals) {
  const safeBody = sanitizeForPrompt(emailBody);
  const safeSubject = sanitizeForPrompt(subject);
  const safeAuthor = sanitizeForPrompt(author);
  const metadataLine = signals ? formatSignalsLine(signals) : "";
...
function buildCombinedExtractionPrompt(emailBody, subject, author, mailDatetime, currentDt, attendeeHints, categories, existingTags, signals) {
  const safeBody = sanitizeForPrompt(emailBody);
  const safeSubject = sanitizeForPrompt(subject);
  const safeAuthor = sanitizeForPrompt(author);
  const metadataLine = signals ? formatSignalsLine(signals) : "";
